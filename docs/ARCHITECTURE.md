...
Путь и схема профиля v1 сохранены. `%LOCALAPPDATA%\IrisOnlineDatabase\WebView2` — стабильный каталог данных WebView2, поэтому состояние, ожидающее записи, сохраняется между запусками. Сборки Windows 7–8.1 используют соседний `WebView2-Windows7`, сохраняя общий файл профиля. Подробнее: `PROFILE_COMPATIBILITY.md` и `WINDOWS_LEGACY.md`.
...
`internal/catalog` содержит подготовку поисковых запросов и ключей сортировки. Запрос готовится один раз; документы предметов, монстров, рецептов, титулов и превращений, а также ключи названий создаются при загрузке неизменяемой базы. Кэш ключей доступен только для чтения после загрузки.

`web/profile.js` содержит безопасный доступ к локальному хранилищу и политику повторов записи. `web/app.js` загружается как ES module. Ошибка профиля сохраняет черновик, показывает уведомление и запускает не более пяти автоматических повторов с увеличивающейся задержкой. Постоянные ошибки требуют ручной попытки.

Смена сервера отменяет предыдущие запросы и заново открывает текущий раздел. Каталог возвращается на первую страницу с сохранением фильтров. Ответ, относящийся к прошлому запросу, не применяется.
