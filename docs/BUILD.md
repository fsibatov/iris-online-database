...
Для нового архива без `.git` начните с `00_RELEASE_WINDOWS.bat` → `PREPARE RELEASE`. Меню восстановит Git-историю и создаст локальный коммит без изменения исходников; это не выполняет push. Проверки и сборка допускаются на чистой `main` или ветке подготовки из `build/release.json` (для 2.1 — `ui-audit-2.1`). Публикация допускается только из `main`.
...
powershell -ExecutionPolicy Bypass -File .\IrisTools.ps1 -Action Build -OutputDirectory C:\IrisRelease\2.1
```
...
Исходник — `build/windows/icon.svg`. Сборка использует `build/windows/icon.ico` с отдельными 32-битными изображениями 16, 20, 24, 32, 40, 48, 64, 128 и 256 px. При изменении SVG экспортируйте каждый размер отдельно и соберите ICO с этими кадрами; одного PNG недостаточно. Проверка ресурсов контролирует размеры, границы кадров и подключение ICO к EXE.
