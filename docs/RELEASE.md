...
3. `IrisTools.ps1 -Action Build -OutputDirectory <внешняя папка>` повторно проверяет fingerprint и создаёт `iris-online-database-2.0.0-windows-amd64.exe` плюс `SHA256SUMS.txt` вне source.
