...
3. `IrisTools.ps1 -Action Build -OutputDirectory <внешняя папка>` повторно проверяет fingerprint и создаёт пять EXE: `IrisOnlineDB-X.Y.Z-Windows-x64.exe`, `IrisOnlineDB-X.Y.Z-Windows-x86.exe`, `IrisOnlineDB-X.Y.Z-Windows-arm64.exe`, `IrisOnlineDB-X.Y.Z-Windows-7-8.1-x64.exe`, `IrisOnlineDB-X.Y.Z-Windows-7-8.1-x86.exe`. Единый `SHA256SUMS.txt` содержит все пять файлов; результаты хранятся вне source.
