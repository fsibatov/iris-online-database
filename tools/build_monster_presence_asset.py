...
def monster_ids(directory: Path) -> list[int]:
    ...
    for path in sorted(directory.glob('*.txt'), key=lambda p: p.name.lower()):
        ...
        while i < len(lines):
            ...
            if i >= len(lines):
                break
            i += 1  # group settings/header; length is variable
...
def main() -> int:
    parser = argparse.ArgumentParser(description='Build server-specific monster visibility from MonsterRegen directories.')
    ...
    print(f"original={len(data['servers']['original'])} kiss={len(data['servers']['kiss'])} output={args.output}")
    return 0
