...
ITEM_PRESENTATION = {
    "id": "technical",
    "name": "header",
    "tooltip": "conditional-description",
    "mainCategoryId": "technical",
    "mainCategory": "technical",
    "middleCategoryId": "technical",
    "middleCategory": "technical",
    "subCategoryId": "technical",
    "subCategory": "technical",
    "category": "header-or-technical",
    "subcategory": "technical",
    "typeLine": "header",
    "qualityId": "technical",
    ...
    "weight": "base-stat",
    "capacity": "technical",
    "sellType": "technical-and-price",
    "price": "price",
    "maxStack": "base-stat",
    "exchange": "technical",
    "seal": "action-and-technical",
    "cardSlots": "card-slots",
    "setIndex": "set-and-technical",
    "iconIndex": "technical",
    "race": "technical",
    "raceName": "restriction-if-labelled",
    "gender": "technical",
    "genderName": "restriction-if-labelled",
    "job1": "technical",
    ...
    "attackSpeed": "base-stat",
    "cooldown": "base-stat",
    "physicalMin": "base-stat",
    "physicalMax": "base-stat",
    "magicMin": "base-stat",
    "magicMax": "base-stat",
    "heal": "base-stat",
    "options": "bonus-stats",
...
ITEM_SUPPLEMENT_PRESENTATION = {
    ...
    "magicMax": "base-stat",
    "heal": "base-stat",
    "options": "bonus-or-neutral-unknown",
    "abilityDescription": "bonus-text",
    "abilityDescriptionIndex": "technical",
    "defenseType": "technical",
    "rangeType": "technical",
    "targetType": "technical",
    "useRange": "technical",
    "groupTime": "technical",
    "influenceIndex": "technical",
    "activeIndex": "technical",
    "effectDurationMs": "base-stat",
    "nameIndex": "technical",
    "tooltipIndex": "technical",
    "abilityIndex": "technical",
    "cardIndex": "technical",
    "useMapType": "restriction",
    "makeSkill": "restriction",
    "makeSkillExp": "restriction",
    "guildUse": "restriction",
    "limitMapTypeRaw": "technical",
    "limitValueRaw": "technical",
    "limitExtraRaw": "technical",
    "kindOf": "technical",
    "eventType": "technical",
    "buyCurrency": "technical",
    "buyPrice": "technical",
    "maxInventory": "restriction",
    "termSet": "restriction-or-technical",
    "termDuration": "restriction-or-technical",
    "printableFlag": "technical",
    "limitIndex": "technical",
    "tarotIndex": "technical",
    "spreadIndex": "technical",
    "degradationIndex": "action-and-technical",
    "cardSlotIndex": "technical",
    "enhanceProbabilityIndex": "technical",
    "enhancedIndex": "action-and-technical",
    "reinforcingIndex": "technical",
    "changeIndex": "technical",
    "titleIndex": "technical",
    "modelIndex": "technical",
    "modelLeftIndex": "technical",
    "gwipyosi": "technical",
}
...
MONSTER_PRESENTATION = {
    ...
    "mp": "base-stat",
    "exp": "technical",
    "moneyBonus": "technical",
    "defense": "base-stat",
    "magicDefense": "base-stat",
    "hit": "base-stat",
    "evasion": "base-stat",
    "criticalDefense": "base-stat",
    "viewRange": "base-stat",
    "importance": "technical",
    "scale": "technical",
    "attackRadius": "base-stat",
    "walkSpeed": "base-stat",
    "runSpeed": "base-stat",
    "aggressive": "status-badge",
    "followRange": "base-stat",
    "recovery": "technical",
}
...
MONSTER_SUPPLEMENT_PRESENTATION = {
    "nameIndex": "technical",
    "noteIndex": "technical",
    "nameHeight": "technical",
    "sourceFlag": "technical",
    "effectScale": "technical",
    "freeMoveRange": "technical",
    "actionStopRatio": "technical",
    "actionWalkRatio": "technical",
    "actionRunRatio": "technical",
    "actionStopTime": "technical",
    "changeMonsterCheck": "technical",
    "followTime": "technical",
    "escapeType": "technical",
    "escapePercent": "technical",
    "recoveryTime": "technical",
}

SERVER_PRESENTATION = {
    "name": "server-selector",
    "questDrops": "metadata",
    "directRulesCount": "metadata",
    "directMonsters": "metadata",
    "directDropEntries": "metadata",
    "directDropSlots": "metadata",
    "worldRulesCount": "metadata",
    "dropListGroups": "metadata",
    "dropListEntries": "metadata",
    "changeProfiles": "metadata",
    "changedChangeProfiles": "metadata",
    "directDropsUpdatedAt": "data-date",
    "dropListsUpdatedAt": "data-date",
    "worldDropsUpdatedAt": "data-date",
    "dropLists": "drop-runtime",
    "directSlots": "drop-runtime",
    "worldRules": "drop-runtime",
}
...
def audit() -> dict:
    ...
    server_keys = {key for server in game["servers"].values() for key in server}
    ...
    checks = {
        ...
        "server_missing_go_fields": sorted(
            server_keys - json_tags(go_source, "ServerData")
        ),
        "effect_spec_missing_go_fields": sorted(
            effect_spec_keys - json_tags(go_source, "ItemEffectSpec")
        ),
        "ability_supplement_unclassified_fields": sorted(
            ability_fields - ITEM_SUPPLEMENT_PRESENTATION.keys()
        ),
        "ability_supplement_missing_item_fields": sorted(ability_fields - item_go),
        "ability_supplement_missing_patch_fields": sorted(
            ability_fields - item_patch_go
        ),
        "monster_supplement_unclassified_fields": sorted(
            monster_detail_fields - MONSTER_SUPPLEMENT_PRESENTATION.keys()
        ),
        "monster_supplement_missing_monster_fields": sorted(
            monster_detail_fields - monster_go
        ),
        "monster_supplement_missing_patch_fields": sorted(
            monster_detail_fields - monster_patch_go
        ),
    }
    ...
    monster_supplement_ids_missing = sorted(
        set(monster_details.get("monsters", {})) - monster_ids, key=int
    )

    item_option_types = collections.Counter(
        option["type"] for item in game["items"] for option in item.get("options", [])
    )
    ...
    unknown_item_effects = {
        key: value
        for key, value in sorted(item_option_types.items())
        if key not in effect_specs
    }
    zero_item_options = sum(
        1
    ...
    zero_item_options += sum(
        1
        for patch in abilities.get("items", {}).values()
        for option in patch.get("options", [])
        if option.get("value") == 0
    )
    ability_patch_field_counts = collections.Counter(
        key for patch in abilities.get("items", {}).values() for key in patch
    )
    monster_patch_field_counts = collections.Counter(
        key for patch in monster_details.get("monsters", {}).values() for key in patch
    )
    unknown_make_skill_codes = sorted(
        {
            patch.get("makeSkill")
            for patch in abilities.get("items", {}).values()
            if patch.get("makeSkill")
        }
        - {1, 2, 3, 4, 5}
    )
    unknown_use_map_codes = sorted(
        {
            patch.get("useMapType")
            for patch in abilities.get("items", {}).values()
            if patch.get("useMapType")
        }
        - {1, 2, 3, 4, 5, 6, 7}
    )
    unknown_guild_use_codes = sorted(
        {
            patch.get("guildUse")
            for patch in abilities.get("items", {}).values()
            if patch.get("guildUse")
        }
        - {1, 2}
    )
    ...
    for set_id, set_data in sets["sets"].items():
        row_keys = [
            ...
            for row in set_data.get("effects", [])
        ]
        ...
        for position, row in enumerate(set_data.get("effects", [])):
            ...
            threshold_lines[required] += len(row.get("options", [])) + (
        ...
        max_rows_same_threshold = max(
            max_rows_same_threshold, max(threshold_rows.values(), default=0)
        )
        max_effect_lines_same_threshold = max(
            max_effect_lines_same_threshold, max(threshold_lines.values(), default=0)
        )

    unknown_set_effects = {
        key: value
        for key, value in sorted(set_option_types.items())
        if key not in effect_specs
    }
    unknown_active_states = {
        key: value
        for key, value in sorted(active_states.items())
        if key not in {1, 2, 3}
    }
    embedded_sets = game["itemSets"]
    embedded_member_set_ids = {
        key for key, value in embedded_sets.items() if value.get("items")
    }
    supplement_set_ids = set(sets["sets"])
    sets_effects_without_published_members = sorted(
        supplement_set_ids - embedded_member_set_ids, key=int
    )
    member_sets_without_effects = sorted(
        embedded_member_set_ids - supplement_set_ids, key=int
    )
    threshold_counts_per_set = [
        len({row["required"] for row in data.get("effects", [])})
        for data in sets["sets"].values()
    ]

    card_slot_types = collections.Counter(
        slot for item in game["items"] for slot in item.get("cardSlots", [])
    )
    ...
    recipe_material_ids = [
        str(row["itemId"])
        for materials in recipes.get("recipes", {}).values()
        for row in materials
    ]
    missing_recipe_material_ids = sorted(
        {value for value in recipe_material_ids if value not in base_items_by_id},
        key=int,
    )
    ...
    for server_key, server_data in chest_contents.get("servers", {}).items():
        ...
        for chest_id, profile in profiles.items():
            ...
            structurally_valid = (
                ...
                and all(
                    isinstance(row.get("itemId"), int)
                    and row["itemId"] > 0
                    and isinstance(row.get("quantity"), int)
                    and row["quantity"] > 0
                    and isinstance(row.get("enhanced"), int)
                    and row["enhanced"] >= 0
                    and isinstance(row.get("threshold"), int)
                    and row["threshold"] > 0
            ...
            if not structurally_valid:
                ...
                chest_threshold_counts = collections.Counter(
                ...
                probability_known = (
                    ...
                    and all(
                        count >= draw_count for count in chest_threshold_counts.values()
                    )
                )
                if not probability_known:
                    chest_probability_unknown_profiles.append(
                        f"{server_key}:{chest_id}"
                    )
            for row in rows:
    ...
    checks.update(
        {
            ...
            "recipe_missing_material_fallback_absent": bool(missing_recipe_material_ids)
            and "Неизвестный предмет (ID %d)" not in go_source,
            ...
            "ability_supplement_not_merged": "mergeItemAbilitySupplement()"
            not in go_source
            or "assets/item_abilities.json.gz" not in go_source,
            "monster_supplement_not_merged": "mergeMonsterDetailSupplement()"
            not in go_source
            or "assets/monster_details.json.gz" not in go_source,
            "set_supplement_not_merged": "mergeSetSupplement()" not in go_source
            ...
            "chest_output_ids_missing_from_game_data": sorted(
                chest_output_ids_missing, key=int
            ),
            "invalid_chest_profiles": sorted(invalid_chest_profiles),
            "chest_probability_unknown_profiles": sorted(
                chest_probability_unknown_profiles
            ),
            "chest_missing_item_fallback_absent": bool(chest_output_ids_missing)
            and "Неизвестный предмет (ID %d)" not in go_source,
            "frontend_has_set_slice_limit": bool(
                re.search(r"set\.(?:effects|items)\s*\.slice\(0,", js)
            ),
            "frontend_uses_generic_properties_array": "const properties =" in js,
            "monster_id_in_suggestion_preview": bool(
                re.search(
                    r"record\.category,\s*record\.typeName.*?ID \$\{record\.id\}",
                    js,
                    re.DOTALL,
                )
            ),
            "frontend_loses_bonus_known_flag": bool(
                re.search(
    ...
    fatal_keys = (
        ...
        "ability_supplement_unclassified_fields",
        "ability_supplement_missing_item_fields",
        "ability_supplement_missing_patch_fields",
        "monster_supplement_unclassified_fields",
        "monster_supplement_missing_monster_fields",
        "monster_supplement_missing_patch_fields",
        "unknown_set_effect_types",
        "unknown_active_states",
        "unknown_card_slot_types",
    ...
    if (
        ...
        or checks["monster_id_in_suggestion_preview"]
        or checks["frontend_loses_bonus_known_flag"]
    ):
    ...
    return {
        ...
        "setsInEmbedded": len(embedded_sets),
        "setsWithPublishedMembers": len(embedded_member_set_ids),
        "setsWithEffectDefinitions": len(sets["sets"]),
        "setEffectRows": len(set_rows),
        "setThresholds": sorted(thresholds),
        "setThresholdRowCounts": dict(sorted(thresholds.items())),
        "activeEffectRows": sum(active_states.values()),
        "activeThresholdCounts": dict(sorted(active_thresholds.items())),
        "setsWithFivePieceEffects": sum(
            1
            for data in sets["sets"].values()
            if any(row["required"] == 5 for row in data.get("effects", []))
        ),
        ...
        "setsEffectDefinitionsWithoutPublishedMembers": len(
            sets_effects_without_published_members
        ),
        ...
        "itemOptionRows": sum(item_option_types.values()),
        "explicitZeroItemOptions": zero_item_options,
        "itemAbilitySupplementItems": len(abilities.get("items", {})),
        "itemAbilitySupplementFieldCounts": dict(
            sorted(ability_patch_field_counts.items())
        ),
        "monsterSupplementMonsters": len(monster_details.get("monsters", {})),
        "monsterSupplementFieldCounts": dict(
        ...
        "maxRecipeMaterials": max(
            (len(rows) for rows in recipes.get("recipes", {}).values()), default=0
        ),
        "restoredAbilityDescriptions": ability_patch_field_counts.get(
            "abilityDescription", 0
        ),
        "restoredEffectDurations": ability_patch_field_counts.get(
            "effectDurationMs", 0
        ),
...
def main() -> None:
    ...
    if args.json:
        ...
        print(
            f"items={result['items']} monsters={result['monsters']} sets={result['setsInEmbedded']}"
        )
        print(
            f"set rows={result['setEffectRows']} thresholds={result['setThresholds']} active={result['activeEffectRows']}"
        )
        print(
            f"5-piece sets={result['setsWithFivePieceEffects']} active@5={result['fivePieceActiveEffects']}"
        )
        print(
