...
def assert_status(base: str, path: str, expected: int = 200, **kwargs):
    ...
    if status != expected:
        raise AssertionError(
            f"{path}: status {status}, expected {expected}; payload={payload!r}"
        )
    return payload
...
def main() -> None:
    ...
    with RunningApp(binary, ["-no-browser"]) as app:
        ...
        assert search["items"] or search["monsters"], (
            "word-form search returned nothing"
        )
        ...
        empty = assert_status(
            app.base_url, "/api/items?q=__definitely_missing__&page=1&pageSize=24"
        )
        ...
        filtered = assert_status(
            app.base_url,
            "/api/items?category=%D0%9E%D1%80%D1%83%D0%B6%D0%B8%D0%B5%2F%D1%89%D0%B8%D1%82&page=1&pageSize=24&sort=level",
        )
        assert filtered["page"] == 1 and len(filtered["items"]) <= 24

        recipes = assert_status(
            app.base_url, "/api/recipes?page=1&pageSize=24&sort=name"
        )
        assert recipes["total"] == 1207, recipes["total"]
        assert len(recipes["recipes"]) <= 24 and recipes["recipes"], (
            "recipe catalog is empty"
        )
        assert recipes["recipes"][0].get("materials"), "recipe row has no materials"
        food_recipes = assert_status(
            app.base_url,
            "/api/recipes?type=%D0%A0%D0%B5%D1%86%D0%B5%D0%BF%D1%82%20%D0%B5%D0%B4%D1%8B&page=1&pageSize=24&sort=mastery",
        )
        assert food_recipes["total"] == 194, food_recipes["total"]
        mastery_values = [
            int(row.get("masteryLevel", 0)) for row in food_recipes["recipes"]
        ]
        assert mastery_values == sorted(mastery_values), mastery_values
        reference_recipe = assert_status(
            app.base_url, "/api/recipes?q=891219&page=1&pageSize=24&sort=mastery"
        )
        reference_rows = [
            row
            for row in reference_recipe["recipes"]
            if int(row.get("id", 0)) == 891219
        ]
        ...
        sourced_recipes = assert_status(
            app.base_url,
            "/api/recipes?knownSource=1&server=kiss&page=1&pageSize=24&sort=name",
        )
        assert sourced_recipes["total"] > 0, (
            "known-source recipe filter returned nothing"
        )
        assert all(
            row.get("sourceCount", 0) > 0 and row.get("sourcePreview", {}).get("name")
            for row in sourced_recipes["recipes"]
        ), "recipe source preview missing"
        paged = assert_status(
            app.base_url, "/api/monsters?page=2&pageSize=24&sort=level"
        )
        assert paged["page"] == 2 and len(paged["monsters"]) <= 24
        kiss_catalog = assert_status(
            app.base_url, "/api/monsters?server=kiss&page=1&pageSize=8"
        )
        original_catalog = assert_status(
            app.base_url, "/api/monsters?server=original&page=1&pageSize=8"
        )
        ...
        assert (
            bounded["page"] == 1
            and bounded["pageSize"] == 48
            and len(bounded["items"]) <= 48
        )
        ...
        while len(keys) < 620:
            data = assert_status(
                app.base_url, f"/api/items?page={page}&pageSize=48&sort=name"
            )
        ...
        chest_source = next(
            (
                row
                for row in chest_item["drops"]
                if row.get("source") == "Сундук" and row.get("containerId") == 808094
            ),
            None,
        )
        ...
        silk_hat = next(
            (
                row
                for row in (chest.get("chest") or {}).get("items", [])
                if row.get("itemId") == 101402
            ),
            None,
        )
        assert (
            silk_hat is not None
            and silk_hat.get("chanceKnown") is True
            and abs(silk_hat["chance"] - 15.204) < 1e-9
        ), silk_hat

        quest_box = assert_status(app.base_url, "/api/items/873063?server=kiss")
        assert (quest_box.get("chest") or {}).get("items"), (
            "non-category item-change box was filtered out"
        )
        ...
        assert all(
            row.get("chanceKnown") is False and "chance" not in row
            for row in anomalous_items
        ), anomalous_items
        ...
        world_source = next(
            (
                row
                for row in world_item["drops"]
                if row.get("source") == "Мировое выпадение"
            ),
            None,
        )
        ...
        assert world_monsters["monsters"], (
            "world source did not expand to level/type candidates"
        )

        hostile = assert_status(app.base_url, "/api/monsters/85?server=kiss")
        assert hostile["monster"]["name"] == "Враждебный дух"
        assert hostile.get("worldRuleCount", 0) > 0, (
            "hostile spirit exposes no world-drop rules"
        )
        hostile_world = assert_status(
            app.base_url, "/api/monster-world-drops?server=kiss&monsterId=85"
        )
        ...
        assert sorted(round(row["baseAttemptChance"], 10) for row in soul_beads) == [
            0.05,
            0.1,
            0.85,
        ], soul_beads
        assert (
            len(golden_chest) == 1
            and abs(golden_chest[0]["baseAttemptChance"] - 0.36) < 1e-12
        ), golden_chest
