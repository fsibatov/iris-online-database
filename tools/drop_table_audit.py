...
def markdown_report(result: AuditResult) -> str:
    lines = [
        ...
        "",
        f"- Монстров с индивидуальными таблицами выпадения: {result.normal_monsters}",
        ...
        f"- Строк с `quantity > 1`: {result.quantity_over_one_rows}",
        f"- Предметов с ограничениями по ID: {result.restricted_items}",
        f"- Строк со штрафом выпадения: {result.penalty_rows}",
        ...
        "",
        "Эталонная модель использует шкалу от 1 до 1 000 000 и накопительные границы сервера.",
        "Поля `field` и `instance` рассматриваются только как условия ветки `Item_DropW` и не используются как связь с конкретным подземельем.",
        "Точный шанс получить предмет за одно убийство нельзя определить только по этим таблицам: на него также влияют серверные штрафы, временные ограничения, условия AM/PM и модификаторы `drop-add`/`event`.",
    ]
    return "\n".join(lines) + "\n"
