...
def build(item_mixed: Path) -> dict:
    ...
    for raw in item_mixed.read_text(encoding="cp1251", errors="replace").splitlines():
        ...
        # item_mixed stores up to six material triplets starting at column 24:
        # item ID, quantity, source-side flag. The client tooltip uses ID/count.
        for position in range(24, 42, 3):
            if position >= len(parts):
                break
            item_id = safe_int(parts[position].strip()) if parts[position].strip() else 0
            quantity = safe_int(parts[position + 1].strip()) if position + 1 < len(parts) and parts[position + 1].strip() else 0
            if item_id:
                ingredients.append({"itemId": item_id, "quantity": max(1, quantity or 1)})
...
def main() -> None:
    ...
    with args.output.open("wb") as output:
        with gzip.GzipFile(filename="", mode="wb", fileobj=output, mtime=0, compresslevel=9) as gz:
            gz.write(payload)
    print(f"recipes={len(data['recipes'])} ingredients={sum(len(rows) for rows in data['recipes'].values())} output={args.output}")
